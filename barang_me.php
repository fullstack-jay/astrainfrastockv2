...
$halaman = "barang_me"; // halaman
$dataapa = "Aset Mechanical Electrical"; // data
...
function confirmDeletion(no) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            setTimeout(() => {
                window.location.href = `component/delete/delete_master?no=${no}&forward=<?php echo $forward; ?>&forwardpage=barang_me&chmod=<?php echo $chmod; ?>`;
...
<div class="box">
    <div class="box-header">
        <?php
        $sqla = "SELECT COUNT(*) AS totaldata FROM $forward WHERE kategori LIKE 'ME%'";
        $hasila = mysqli_query($conn, $sqla);
        $rowa = mysqli_fetch_assoc($hasila);
        $totaldata = $rowa['totaldata'];
        ?>
